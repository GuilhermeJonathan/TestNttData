...
public class ListProductsHandler : IRequestHandler<ListProductsCommand, ListProductsResult>
{
    ...
    public async Task<ListProductsResult> Handle(ListProductsCommand request, CancellationToken cancellationToken)
    {
        ...
        var products = await _productRepository.GetPagedProductsAsync(request.Page, request.Size, request.Order, orderDescending, null,
            cancellationToken);

        var totalProducts = await _productRepository.GetTotalCountAsync(null, cancellationToken);
